...
int main()
{
    ...
    while(1)
    {
	    ...
	    if(FD_ISSET(client_fd,&rfd_set))
	       ...
	       if(strstr(buff,"I am ")!=NULL)
	       {
		  Num++;
                  client_ph[Num-1]=(char*)malloc(sizeof(char)*10);
                  for(int i=5;i<=14;i++)client_ph[Num-1][i-5]=buff[i];
		  ...
		  memset(buff,'\0',MAXSIZE);
	          ...
	          for(int i=0;i<Num;i++)
		  {
		     if(strstr(buff,client_ph[i])!=NULL)sem_post(&costumer[i]);
		  }
               }
	       memset(buff,'\0',MAXSIZE);
            }
